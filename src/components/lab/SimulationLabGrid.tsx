...
export const SimulationLabGrid: React.FC<Props> = ({ focusSimId }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-10">
      ...
      {activeSim && (
        <section className="p-6 bg-slate-900/95 border-2 border-emerald-500/50 rounded-2xl shadow-2xl space-y-4 animate-fadeIn ring-2 ring-emerald-500/20">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800">
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs px-2.5 py-0.5 rounded bg-emerald-500/20 text-emerald-300 font-mono font-semibold">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-white px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 transition self-start sm:self-auto font-mono"
            >
      ...
      {/* 3. Section 1: Các Dạng Tương Tác Trong Thí Nghiệm */}
      <section>
        <InteractionPatternsGuide />
      </section>

      {/* 4. Section 2: Bảng Danh Mục Thí Nghiệm Chuẩn GDPT 2018 (Mầm non - THPT) */}
      <section>
        <SimulationCatalogTable onRunLiveSimulation={handleRunLiveSim} />
      </section>

      {/* 5. Section 3: Thẻ Khởi Động Nhanh Các Mô Phỏng Tương Tác Sẵn Có */}
      <section className="space-y-4 bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <h3 className="font-bold text-slate-200 text-base flex items-center gap-2 font-mono">
              <Sparkles className="w-4 h-4 text-sky-400" />
              <span>Phòng Lab Tương Tác Nổi Bật (Live WebGL/Canvas Engines)</span>
            </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {SIMULATIONS.map((sim) => {
            ...
            return (
              <div
                key={sim.id}
                className={`flex flex-col justify-between bg-slate-950/70 border rounded-xl p-4 hover:border-slate-700 transition space-y-3 ${
                  isSelected ? 'border-emerald-500/70 ring-1 ring-emerald-500/40' : 'border-slate-800'
                }`}
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-850 text-slate-300">
                      {sim.gradeLabel}
                    </span>
                    <span className="text-[10px] text-slate-400 font-mono">{sim.subjectName}</span>
                  </div>

                  <h4 className="font-semibold text-slate-100 text-xs sm:text-sm leading-snug">
                    {sim.title}
                  </h4>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-1.5 py-1.5 px-3 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 hover:text-emerald-300 border border-emerald-500/30 text-xs font-semibold font-mono transition"
                >
