...
export const SidebarLessonTree: React.FC<Props> = ({
  ...
  return (
    <aside className="w-full lg:w-72 flex-shrink-0 bg-white dark:bg-[#0c101a] border-r border-slate-200 dark:border-slate-800 p-4 space-y-4 transition-colors">
      {/* Title */}
      <div className="flex items-center justify-between pb-2 border-b border-slate-200 dark:border-slate-800/80">
        <div className="flex items-center gap-2">
          <BookCheck className="w-4 h-4 text-sky-600 dark:text-sky-400" />
          <h3 className="font-semibold text-xs uppercase tracking-wider text-slate-700 dark:text-slate-300 font-mono">
      ...
      <div className="space-y-4 text-xs">
        {chapters.length === 0 ? (
          <p className="text-slate-400 italic py-4">Đang cập nhật nội dung chương trình...</p>
        ) : (
          chapters.map((chap) => {
            ...
            return (
              <div key={chap.id} className="space-y-1">
                {/* Chapter Title */}
                <div className="flex items-start gap-1.5 py-1 text-slate-700 dark:text-slate-400 font-semibold leading-snug">
                  <Folder className="w-3.5 h-3.5 text-sky-500/80 flex-shrink-0 mt-0.5" />
                  <span className="text-[11px] font-mono text-slate-800 dark:text-slate-300">{chap.title}</span>
                </div>

                {/* Lessons in Chapter */}
                <div className="pl-4 space-y-0.5 border-l border-slate-200 dark:border-slate-800/80 ml-1.5">
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left flex items-start gap-2 p-2 rounded-lg transition ${
                          isSelected
                            ? 'bg-sky-50 dark:bg-sky-500/20 text-sky-800 dark:text-sky-300 font-medium border border-sky-300 dark:border-sky-500/30'
                      ...
                      >
                        ...
                        {hasSim && (
                          <span
                            ...
                            className="p-0.5 text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 rounded flex-shrink-0"
                          >
