...
export const SubLevelNav: React.FC<Props> = ({
  ...
  return (
    <div className="bg-[#0e1422] border-b border-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 space-y-2.5">
        {/* Level Overview Philosophy Banner */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 p-2.5 rounded-lg bg-slate-900/60 border border-slate-800/80 text-xs">
          <div className="flex items-start md:items-center gap-2">
            <span className="px-2 py-0.5 rounded bg-sky-500/10 text-sky-400 font-mono font-semibold text-[11px] whitespace-nowrap">
          ...
          <span className="text-slate-400 text-[11px] font-mono whitespace-nowrap">
            Nguồn: <strong className="text-slate-300">{levelInfo.bookSeries}</strong>
          </span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 pt-1">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[11px] font-mono text-slate-500 uppercase mr-1">Khối:</span>
            {grades.map((grade) => {
          ...
          {/* Subject Selector */}
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[11px] font-mono text-slate-500 uppercase mr-1">Môn học:</span>
            {subjects.length === 0 ? (
              <span className="text-xs text-slate-500 italic">Chưa có môn học cho khối này</span>
            ) : (
              subjects.map((sub) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-medium border transition ${
                      isSelected
                        ? 'bg-sky-950/50 border-sky-500/50 text-sky-300 shadow-sm'
